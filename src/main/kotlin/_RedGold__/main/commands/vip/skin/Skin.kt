...
class Skin(private val listener: SkinPacketListener) : TabExecutor {
    override fun onCommand(
        ...
        if ((cooldownMap[uuid]?: 0L) > now) {
            player.fail("&c${(cooldownMap[uuid]?: 0L) - now}초 후에 변경이 가능합니다.")
            return true
        }

        taskAsync {
            val (value, signature) = listener.getSkinData(args[0]) ?: run {
                task { player.fail("&c스킨 데이터를 가져오지 못했습니다.") }
                return@taskAsync
            }

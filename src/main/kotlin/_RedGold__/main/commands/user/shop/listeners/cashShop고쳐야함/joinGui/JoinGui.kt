...
class JoinGui(private val plugin: JavaPlugin) {
    ...
    fun openGui(player: Player, sound: Float = 1f) {
        ...
        fun Int.fi(title: String, pur: Int, type: Int) {
            ...
            ).apply {if (joinMsg == type) addUnsafeEnchantment(Enchantment.PROTECTION, 1)})
        }
        ...
        10.fi("기본", 0, 0) //
        11.fi("&8[&a+&8] &ePlayer 님이 서버에 나타났습니다.", 14, 1)
        12.fi("&8[&a+&8] &f&lPlayer &7&lJoined", 14, 2)
        13.fi("&8[&b✦&8] &ePlayer &e님 환영합니다!", 14, 3)
        14.fi("&8[&a»&8] &ePlayer 님이 &a&l온라인&e으로 전환했습니다.", 15, 4)
        15.fi("&8[&7»&8] &fJo&ki&fned with Player", 14, 5)
        16.fi("&8[&b»&8] &f&lPlayer &b&l님이 서버에 등장하였습니다.", 15, 6)

        19.fi("&8[&a+&8] &ePlayer 님이 서버에 &a&l생성되었습니다.", 14, 7)
        20.fi("&8[&7?&8] &ePlayer 님이 서버에 &8&l접속...했나요?", 16, 8)
        21.fi("&8[&a!&8] &ePlayer 님이 게임에 참여하였습니다!", 18, 9)
        22.fi("&8[&a+&8] &ePlayer 님이 &2&l마인크래프트 세상에 들어왔습니다.", 17, 10)
        23.fi("&8[&a+&8] &ePlayer 님이 서버에 접속하였습니다. &a&l환영해주세요!", 18, 11)
        24.fi("&8[&a+&8] &ePlayer", 14, 12)
        25.fi("&8+ &ePlayer", 14, 13)

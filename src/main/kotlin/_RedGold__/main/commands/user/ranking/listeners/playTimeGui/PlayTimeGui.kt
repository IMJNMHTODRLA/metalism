...
class PlayTimeGui {
    fun openGui(player: Player, page: Int) {
        ...
        val playTimeRank = GlobalValue.playTimeRank
        ...
        repeat(16) { i ->
            ...
            if (ranking >= playTimeRank.size) return@repeat
            val (_, key, value) = playTimeRank[ranking]

            val offlinePlayer = Bukkit.getOfflinePlayer(key)
            val name = offlinePlayer.name?: "알 수 없음"

            val formatRankingInfo = PlayTimeConst.getFormatRanking(ranking)
            gui.item[slot] = PlayTimeConst.getRankingIcon(
                key, name,
        ...
        val playerRanking = playTimeRank[uuid]

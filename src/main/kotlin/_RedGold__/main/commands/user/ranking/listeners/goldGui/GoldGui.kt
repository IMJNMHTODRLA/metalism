...
class GoldGui {
    fun openGui(player: Player, page: Int) {
        ...
        val goldRank = GlobalValue.goldRank
        ...
        repeat(16) { i ->
            ...
            if (ranking >= goldRank.size) return@repeat
            val (_, key, value) = goldRank[ranking]

            val offlinePlayer = Bukkit.getOfflinePlayer(key)
            val name = offlinePlayer.name?: "알 수 없음"

            val formatRankingInfo = GoldConst.getFormatRanking(ranking)
            gui.item[slot] = GoldConst.getRankingIcon(
                key, name,
        ...
        val playerRanking = goldRank[uuid]

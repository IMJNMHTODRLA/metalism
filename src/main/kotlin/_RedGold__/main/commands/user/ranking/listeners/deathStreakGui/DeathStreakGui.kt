...
class DeathStreakGui {
    fun openGui(player: Player, page: Int) {
        val gui = DeathStreakHolder(page).inventory
        val uuid = player.uniqueId
        val deathStreakRank = GlobalValue.deathStreakRank
        ...
        repeat(16) { i ->
            ...
            if (ranking >= deathStreakRank.size) return@repeat
            val (_, key, value) = deathStreakRank[ranking]

            val offlinePlayer = Bukkit.getOfflinePlayer(key)
            val name = offlinePlayer.name?: "알 수 없음"

            val formatRankingInfo = DeathStreakConst.getFormatRanking(ranking)
            gui.item[slot] = DeathStreakConst.getRankingIcon(
                key, name,
        ...
        val playerRanking = deathStreakRank[uuid]

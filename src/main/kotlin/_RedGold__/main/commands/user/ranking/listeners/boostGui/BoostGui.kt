...
class BoostGui {
    fun openGui(player: Player, page: Int) {
        ...
        val boostRank = GlobalValue.boostRank
        ...
        repeat(16) { i ->
            ...
            if (ranking >= boostRank.size) return@repeat
            val (_, key, value) = boostRank[ranking]

            val offlinePlayer = Bukkit.getOfflinePlayer(key)
            val name = offlinePlayer.name?: "알 수 없음"

            val formatRankingInfo = BoostConst.getFormatRanking(ranking)
            gui.item[slot] = BoostConst.getRankingIcon(
                key, name,
        ...
        val playerRanking = boostRank[uuid]

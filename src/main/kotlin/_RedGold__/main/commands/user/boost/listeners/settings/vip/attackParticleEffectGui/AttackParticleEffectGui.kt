...
class AttackParticleEffectGui {
    fun openGui(player: Player, page: Int) {
        val gui = AttackParticleEffectHolder(page).inventory
        gui.item(BACKGROUND)
        gui.item[27..gui.end] = BACKGROUND_1

        val hasRank = player in ATTACK_PARTICLE_EFFECT
        val equipValue = player.data.boostSettingMap[ATTACK_PARTICLE_EFFECT]?: ATTACK_PARTICLE_EFFECT.default
        ...
        repeat(14) { i ->
            ...
            gui.item[slot] = SettingsGlobalConst.setDisplayItem(
                ...
                equipValue == id,
                hasRank,
                ATTACK_PARTICLE_EFFECT.needPermission
            )
        }

...
class ChatGGColorGui {
    fun openGui(player: Player, page: Int) {
        val gui = ChatGGColorHolder(page).inventory
        gui.item(BACKGROUND)
        gui.item[27..gui.end] = BACKGROUND_1

        val hasRank = player in CHAT_GG_COLOR
        val equipValue = player.data.boostSettingMap[CHAT_GG_COLOR]?: CHAT_GG_COLOR.default

        repeat(14) { i ->
            val slot = SettingsGlobalConst.getSlot(i)
            val id = SettingsGlobalConst.getIdFromI(page, i)
            val data = chatGGColorList.getOrNull(id)?: return@repeat

            gui.item[slot] = SettingsGlobalConst.setDisplayItem(
                data.first,
                equipValue == id,
                hasRank,
                CHAT_GG_COLOR.needPermission
            )
        }

...
class TicketGui(private val plugin: JavaPlugin) {
    ...
    fun openGui(player: Player, sound: Float = 1f) {
        ...
        gui.setItem(13, getItem(
            "emerald",
            "&a뽑기권 구매 또는 사용 &8구매 횟수: ($buyTimes/4)",
            listOf(
                "",
                "&a[구매(좌클릭)] &f&l구매가: 150 캐시 &8&l(5연속 뽑기권이 구매 됩니다.)",
                "&b[사용(우클릭)] &f&l5연속 뽑기권이 사용됩니다.&8(사용 가능한 뽑기권 횟수: &a&l${getTicket}회)",
                "",
                "&f&l뽑기에서 각 회차마다 개별적으로 25%의 확률로 치장품이 등장 합니다.",
                "&f&l75% 확률로 실패 할 시 100,000 골드가 지급됩니다.",
                "&f&l만일 25% 확률로 성공 시 치장품에서 사망/킬 사운드는 각각 8.75%,",
                "&f&l접속 메시지는 5%, 칭호(영구 획득)는 2.5% 확률로 등장합니다.",
                "",
